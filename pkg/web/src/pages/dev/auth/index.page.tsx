...
import { useApi, useApiWithSigning } from '@violet/web/src/hooks'
...
const Home = () => {
  const { api } = useApi()
  const { api: apiWithSigning } = useApiWithSigning()
  const { data: comments, mutate } = useAspidaSWR(api.dev.comment)
  const [isSignInModalOpen, setIsSignInModalOpen] = useState(false)
  const { currentUser, initialized, signOut } = useAuth()

  const onSubmit = async (ev: FormEvent) => {
    ev.preventDefault()
    await api.dev.comment.$post({ body: 'foo' })
    await mutate()
  }

  const submitWithSigning = async () => {
    await apiWithSigning.dev.comment.$post({ body: 'foo' })
    await mutate()
  }
  ...
  const Form = () => (
    <form action="" onSubmit={onSubmit}>
      ...
      <Fieldset>
        ...
        <button onClick={submitWithSigning}>自動リトライつきリクエスト</button>
      </Fieldset>
    </form>
  )
